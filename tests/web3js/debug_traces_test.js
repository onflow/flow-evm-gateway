...
it('should retrieve transaction traces', async () => {
    ...
    response = await helpers.callRPCMethod(
        ...
        [receipt.transactionHash, tracerConfig]
    )
    ...
    response = await helpers.callRPCMethod(
        ...
        [web3.utils.toHex(receipt.blockNumber), tracerConfig]
    )
    assert.equal(response.status, 200)
    assert.isDefined(response.body.result)
    ...
    response = await helpers.callRPCMethod(
        ...
        [web3.utils.toHex(receipt.blockHash), tracerConfig]
    )
    assert.equal(response.status, 200)
    assert.isDefined(response.body.result)
