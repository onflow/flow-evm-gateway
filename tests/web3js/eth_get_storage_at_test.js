...
it('deploy contract and interact', async () => {
    ...
    let stored = await web3.eth.getStorageAt(contractAddress, slot, latestHeight)
    let value = web3.utils.hexToNumberString(stored)
    ...
    stored = await web3.eth.getStorageAt(contractAddress, slot, res.receipt.blockNumber)
    value = web3.utils.hexToNumberString(stored)
    assert.equal(value, '100')

    stored = await web3.eth.getStorageAt(contractAddress, slot, res.receipt.blockNumber - 1n)
    value = web3.utils.hexToNumberString(stored)
    assert.equal(value, '1337')

    stored = await web3.eth.getStorageAt(contractAddress, slot, res.receipt.blockNumber - 2n)
    value = web3.utils.hexToNumberString(stored)
    assert.equal(value, '1337')

    stored = await web3.eth.getStorageAt(contractAddress, slot, res.receipt.blockNumber - 3n)
    value = web3.utils.hexToNumberString(stored)
