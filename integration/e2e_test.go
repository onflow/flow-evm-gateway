...
func TestIntegration_API_DeployEvents(t *testing.T) {
	...
	rpcURL := fmt.Sprintf("http://%s:%d/rpc", cfg.RPCHost, cfg.RPCPort)
	...
	_, err = newContract(testContractBinary, testContractABI)
	require.NoError(t, err)
	...
	// block 3 comes from contract deployment
	rpcRes, err := rpcRequest(rpcURL, `{"jsonrpc":"2.0","id":0,"method":"eth_getBlockByNumber","params":["0x3",false]}`)
